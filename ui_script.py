...
try:
    from PyQt5.QtWidgets import (
        ...
        QPushButton, QLabel, QTextEdit, QComboBox, QProgressDialog,
        QFileDialog, QMessageBox, QStyle,
        QTableView, QSizePolicy, QCheckBox,
        QToolButton, QMenu, QToolBar, QStyleOptionViewItem,
    ...
    from PyQt5.QtCore import (
        ...
        QTimer, QSize, QPoint,
        QModelIndex, QSortFilterProxyModel, QLockFile, QDir,
    )
...
from browser_cookie_profiles import (
    ...
    migrate_cookie_source_settings,
    resolve_download_cookie_source,
    set_cookie_source_mode,
)
from managed_chrome import ManagedChromeSession
try:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._managed_chrome_shutdown = False
        ...
        if migrate_cookie_source_settings(self.settings):
        ...
        self.managed_chrome = ManagedChromeSession(
            os.path.join(SCRIPT_BASE_DIR_UI, DATA_DIR_NAME, "browser")
        )
    ...
    def _can_start_update(self, manual: bool, requested: str | None = None) -> bool:
        ...
        if active_download:
            message = (
                "Đang có tác vụ download diễn ra. "
                "Vui lòng chờ tác vụ đó hoàn thành."
            )
            self.log_to_gui(message)
            if manual:
                QMessageBox.information(self, "Đang bận", message)
            return False
        if (requested != "yt-dlp" and yt_update_running) or (
            requested != "app" and app_update_running
        ):
            message = "Đang có tác vụ cập nhật khác. Vui lòng chờ tác vụ hoàn tất."
            self.log_to_gui(message)
            if manual:
                QMessageBox.information(self, "Đang bận", message)
            return False
    ...
    def _open_managed_browser(self):
        result = self.managed_chrome.open_browser()
        if result.success:
            set_cookie_source_mode(self.settings, COOKIE_SOURCE_MANAGED)
            main_logic.save_settings_to_file(self.settings, GUI_SETTINGS_FILE_PATH)
            self.log_to_gui(result.message)
            return
        message = result.message or "Không thể mở Chrome riêng của ứng dụng."
        self.log_to_gui(f"Không thể mở Trình Duyệt: {message}")
        QMessageBox.warning(self, "Không thể mở Trình Duyệt", message)

    def _resolve_download_cookie_source(self, task_id=None):
        mode = get_cookie_source_mode(self.settings)
        if mode != COOKIE_SOURCE_MANAGED:
            return resolve_download_cookie_source(self.settings)

        managed_source = self.managed_chrome.resolve_cookie_source(
            lease_id=str(task_id) if task_id else None
        )
        source = resolve_download_cookie_source(
            self.settings,
            managed_source=managed_source,
        )
        if (
            not managed_source.use_cookie_file
            and not managed_source.browser_spec
            and self.managed_chrome.profile_dir.exists()
            and self.managed_chrome.last_error
        ):
            self.log_to_gui(
                "Cookie từ Chrome riêng chưa khả dụng: "
                f"{self.managed_chrome.last_error}"
            )
        return source
    ...
    def _init_ui(self):
        ...
        self.managed_browser_action = QAction(
            QIcon.fromTheme(
        ...
        self.managed_browser_action.setToolTip(
            "Mở Chrome riêng của ứng dụng để đăng nhập YouTube"
        )
        self.managed_browser_action.triggered.connect(self._open_managed_browser)
        toolbar.addAction(self.managed_browser_action)
    ...
    def _open_settings_dialog(self):
        self.settings_dialog_instance = SettingsDialog(
            self.settings,
            self,
            managed_chrome=self.managed_chrome,
        )
...
# Thêm hai hàm mới này vào trong lớp MainWindow
    ...
    def _confirm_cookie_mode_for_downloads(self, tasks_to_add):
        ...
        if get_cookie_source_mode(self.settings) == COOKIE_SOURCE_NONE:
            return True
        self.log_to_gui(
            "Cookie đã được cấu hình làm fallback; lần tải đầu vẫn chạy không cookie."
        )
        return True
    ...
    def _on_task_finished(self, task_id, final_message, success_status, downloaded_file_path, error_details):
        ...
        finished_task = self.active_threads.get(task_id)
        if finished_task:
            cookie_source = finished_task.get("cookie_source")
            if cookie_source is not None:
                self.managed_chrome.release_cookie_source(cookie_source)
        if self._shutdown_in_progress:
            return
        row_index = self._find_row_for_task_id(task_id)
    ...
    def _process_download_queue(self):
        ...
        for task_id, task_info in queued_tasks:
            ...
            if task_type == "drive_folder":
                ...
                cookie_source = self._resolve_download_cookie_source(task_id)
                task_info["cookie_source"] = cookie_source
                use_cookies_setting = cookie_source.use_cookie_file
                cookies_file_path_setting = cookie_source.cookie_file_path
                cookies_from_browser_setting = cookie_source.browser_spec
                if cookie_source.invalid_cookie_file:
                    self.log_to_gui(
                        "File cookie đã chọn không có phiên đăng nhập YouTube hợp lệ; "
                        "bỏ qua fallback cookie cho tác vụ này."
                    )
                ...
                self._track_thread(
                    ...
                    worker,
                    completion_signals=(worker.task_finished,),
                    cleanup_callback=lambda source=cookie_source: (
                        self.managed_chrome.release_cookie_source(source)
                    ),
                )
                task_info["thread"] = thread
                task_info["worker"] = worker
    ...
    def _on_task_log_message(self, task_id, message):
        if "DEBUG:" in message or "Lỗi" in message or "STDERR:" in message or "Lệnh:" in message or "Bắt đầu tải:" in message or "Workspace" in message or "cookie lần cuối" in message:
    ...
    def _show_error_detail_dialog(self, task_id, error_details):
        ...
        copy_reset_timer = QTimer(copy_btn)
        copy_reset_timer.setSingleShot(True)
        copy_reset_timer.timeout.connect(lambda: copy_btn.setText("  Sao chép"))

        def _copy_error():
            QApplication.clipboard().setText(error_details)
            copy_btn.setText("  Đã sao chép ✓")
            copy_reset_timer.start(2000)
    ...
    def _finish_deferred_close(self):
        ...
        self._shutdown_managed_chrome_once()
        self._allow_final_close = True
        self.close()

    def _shutdown_managed_chrome_once(self):
        if self._managed_chrome_shutdown:
            return
        self._managed_chrome_shutdown = True
        try:
            self.managed_chrome.shutdown()
        except Exception as exc:
            self.log_to_gui(f"Không thể đóng Chrome riêng hoàn toàn: {exc}")
