...
from PyQt5.QtCore import Qt, QThread, pyqtSignal as Signal, QObject, pyqtSlot as Slot, QTimer
...
class DownloadWorker(QObject):
    ...
    def run(self):
        ...
        command_args = [
            ...
            '--no-check-certificate', # << THÊM DÒNG NÀY ĐỂ BỎ QUA LỖI SSL
        ]
        ...
        # Cookies: file-based
        if self.use_cookies and self.cookies_file_path and os.path.exists(self.cookies_file_path):
            self.log_message.emit(self.task_id, f"Sử dụng file cookies tại: {self.cookies_file_path}")
            command_args.extend(['--cookies', self.cookies_file_path])
        elif self.use_cookies:
            self.log_message.emit(self.task_id, f"CảNH BÁO: Đã bật tùy chọn dùng cookies nhưng đường dẫn file không hợp lệ: '{self.cookies_file_path}'")
        # Cookies: from browser
        if self.cookies_from_browser:
            self.log_message.emit(self.task_id, f"Lấy cookies từ trình duyệt: {self.cookies_from_browser}")
            command_args.extend(['--cookies-from-browser', self.cookies_from_browser])
        ...
        if is_audio_request:
            ...
            command_args.append(self.url)
        # ... (phần còn lại của run: thực thi lệnh, parse output, xử lý thumbnail, emit signals giữ nguyên) ...
        try:
            log_command_str = subprocess.list2cmdline(command_args)
        except AttributeError:
            log_command_str = ' '.join(f'"{arg}"' if ' ' in arg else arg for arg in command_args)
        self.log_message.emit(self.task_id, f"Lệnh yt-dlp: {log_command_str}")
        ...
        try:
            ...
            if process.stdout:
                for line in iter(process.stdout.readline, ''):
                    ...
                    self.log_message.emit(self.task_id, f"STDOUT: {line.strip()}")
            ...
            if process.stderr:
                for line in iter(process.stderr.readline, ''):
                    if not self._is_running:
                        break
                    error_details_buffer.append(line.strip())
                    self.log_message.emit(self.task_id, f"STDERR: {line.strip()}")
            ...
            final_error_details_str = "\n".join(error_details_buffer[-10:]) # Lấy 10 dòng lỗi cuối
