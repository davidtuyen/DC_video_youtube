...
def _is_package_protected(relative_text: str) -> bool:
    canonical = _canonical_windows_path(relative_text)
    return canonical in DEFAULT_PRESERVED_PATHS or canonical.startswith((
        "data/browser/",
        "data/thumbnails/",
        "yt-dlp/",
    ))
    ...
    def validate_package(zip_path: str | os.PathLike[str]) -> dict:
        with zipfile.ZipFile(zip_path, "r") as archive:
            ...
            for entry in manifest["files"]:
                ...
                if _is_package_protected(canonical):
            ...
            if protected_paths:
                raise ValueError(
                    "update package contains protected paths: "
