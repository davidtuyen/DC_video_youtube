...
class PublishPackagingTests(unittest.TestCase):
    def _make_dist(self, root: Path, runtime_tag="1.0.14", updater_tag="1.0.14"):
        ...
        browser_cookie = app_dir / "data" / "browser" / "profile" / "Cookies"
        browser_cookie.parent.mkdir(parents=True)
        browser_cookie.write_bytes(b"legacy private browser session")
        managed_cookie = (
            app_dir
            / "data"
            / "browser"
        ...
        managed_cookie.parent.mkdir(parents=True)
        managed_cookie.write_bytes(b"managed login database")
        managed_cache = managed_cookie.parents[1] / "Cache" / "cache-entry"
        managed_cache.parent.mkdir(parents=True)
        managed_cache.write_bytes(b"managed cache")
        (app_dir / "data" / "browser" / "session-cookies.txt").write_text(
            "plaintext snapshot",
            encoding="utf-8",
        )
    ...
    def test_runtime_smart_package_requires_v1015_and_complete_runtime(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertNotIn("data/browser/profile/Cookies", entries)
            self.assertFalse(
                any(path.startswith("data/browser/") for path in entries)
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(publish, "GITHUB_TOKEN", "test-token"):
                with mock.patch.object(
                    publish, "_git_release_commit", return_value=release_commit, create=True
                ):
                    with mock.patch.object(
                        publish.requests,
                        "post",
            ...
            self.assertEqual(
                post.call_args_list[0].kwargs["json"]["target_commitish"],
                release_commit,
            )
    ...
    def test_release_target_uses_the_tag_commit_when_the_tag_exists(self):
        tagged_commit = "b" * 40
        tag_result = subprocess.CompletedProcess([], 0, tagged_commit + "\n", "")

        with mock.patch.object(publish.subprocess, "run", return_value=tag_result) as run:
            resolved = publish._git_release_commit("1.0.17")

        self.assertEqual(resolved, tagged_commit)
        self.assertIn("refs/tags/1.0.17^{commit}", run.call_args.args[0])
    ...
    def test_v1015_release_metadata_and_notes(self):
        ...
        self.assertEqual(source_version, "1.0.18")
        self.assertEqual(publish.UPDATER_WORKER_VERSION, "2.1.0")
        self.assertEqual(publish.UPDATER_WORKER_RELEASE_TAG, "1.0.15")
        self.assertIn('#define MyAppVersion "1.0.18"', installer)
        self.assertIn("LICENSE", body)
        self.assertIn("yt-dlp 2026.07.04", body)
        self.assertIn("UpdaterWorker 2.1.0", body)

    def test_v1017_release_notes_explain_cleanup_and_packaged_runtime_fixes(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            setup = Path(temp_dir) / "YouTubeDownloaderPro-Setup-v1.0.17.exe"
            smart = Path(temp_dir) / "app-update-v2.pkg"
            setup.write_bytes(b"setup")
            smart.write_bytes(b"smart")

            body = publish._release_body("1.0.17", [setup, smart])

        self.assertIn("dead code", body)
        self.assertIn("FFmpeg/FFprobe", body)
        self.assertIn("settings và lịch sử tải", body)

    def test_v1018_release_notes_explain_google_drive_oauth_fix(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            setup = Path(temp_dir) / "YouTubeDownloaderPro-Setup-v1.0.18.exe"
            smart = Path(temp_dir) / "app-update-v2.pkg"
            setup.write_bytes(b"setup")
            smart.write_bytes(b"smart")

            body = publish._release_body("1.0.18", [setup, smart])

        self.assertIn("OAuth Google Drive", body)
        self.assertIn("Smart Update", body)
        self.assertIn("mở trình duyệt mặc định", body)
    ...
    def test_create_validation_error_is_not_misreported_as_missing_release(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            asset = Path(temp_dir) / "app-update-v2.pkg"
            asset.write_bytes(b"package")
            create_response = mock.Mock(status_code=422, text="invalid target_commitish")
            existing_response = mock.Mock(status_code=404, text="Not Found")
                ...
                with mock.patch.object(publish.requests, "post", return_value=create_response):
                    with mock.patch.object(publish.requests, "get", return_value=existing_response):
                        with self.assertRaisesRegex(
                            RuntimeError,
                            "create draft release.*invalid target_commitish",
                        ):
                            publish.upload_to_github([asset], "1.0.17")
    ...
    def test_v1017_hosts_the_runtime_repair_asset_on_its_own_release(self):
        self.assertEqual(publish.NODE_RUNTIME_RELEASE_TAG, "1.0.17")
    ...
    def test_installer_script_preserves_user_state_and_existing_ytdlp(self):
        ...
        self.assertIn(r"data\browser\*", script)
    ...
    def test_main_app_build_does_not_collect_unused_ytdlp_python_modules(self):
        updater_outputs = (Path("launcher.exe"), Path("worker.exe"), {})

        with tempfile.TemporaryDirectory() as temp_dir:
            with mock.patch.object(publish, "_run_pyinstaller") as run:
                with mock.patch.object(publish.shutil, "copytree") as copytree:
                    with mock.patch.object(publish, "DIST_DIR", Path(temp_dir) / "dist"):
    ...
    def test_main_app_build_stages_google_drive_oauth_client_for_updates(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            dist_dir = root / "dist"
            source_data = root / "data"
            source_data.mkdir()
            credentials = b'{"installed":{"client_id":"team-app"}}'
            (source_data / "credentials.json").write_bytes(credentials)
            (source_data / "runtime-manifest.json").write_text(
                json.dumps({"release_tag": "1.0.17"}), encoding="utf-8"
            )
                ...
                with mock.patch.object(publish, "DIST_DIR", dist_dir):
                    with mock.patch.object(publish, "_run_pyinstaller", side_effect=fake_pyinstaller):
                        app_dir = publish.build_main_app(
                            "1.0.18", (launcher, worker, updater_manifest), verified
                        )

            packaged_client = app_dir / "data" / "google_drive_client.json"
            self.assertEqual(packaged_client.read_bytes(), credentials)
            self.assertFalse((app_dir / "data" / "credentials.json").exists())

            with mock.patch.object(publish, "DIST_DIR", dist_dir):
                package = publish.create_package("smart", "1.0.18")
            manifest = UpdatePackageApplier.validate_package(package)
            paths = {entry["path"] for entry in manifest["files"]}
            self.assertIn("data/google_drive_client.json", paths)
